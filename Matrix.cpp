...
using namespace std;
	...
	void Matrix::In_mask(string in)
	{
		...
		for(int i = 0;i<in.length();i++)
		{
			...
			if(in[i] == '!')
			{
				mask_input[i-negations] = 'N';
				negations++;
			}
			else
			{
				input[i-negations] = in[i];
				in_size++;
			}
			if(in[i] == 'y')
			mask_input[i-negations] = 'y';
		}
	}
			...
			for(int j = 1 ;j<c_size;j++)
					if(isAlpha(L[i][j]))
						for(int k = j+1 ;k<c_size;k++)
							if(isAlpha(L[i][k]) and k-j>=2 and k!=col_y-1)
								{
									L[i][k-1] = "O";
									break;
								}
					
					//!="-" and L[i][j]!="S" and L[i][j+1] == "-" and L[i][j+2]!="-" and first_letter[i+1]<=j)
					//L[i][j+1] = "O";
						...
						for(int k = i+1;k<r_size;k++)
							{
								...
								if(L[k][j] == "-" and last_letter[k]!=-5 and !or_here)
								L[k][j]="o";
							}
					}
		}
		...
		for(int k = 0;k<r_size;k++)
			...
			max = -5;
					...
					for(int z =1;z<r_size;z++){
						for(int h = 1;h<c_size;h++)
						{
							if(isAlpha(L[z][h]) and  h!=col_y and h>max)
									max = h;
						}
						last_letter[z] = max;
					max = -5;
	...
	while(starting<n_y) // finchè non hai risolto tutte le equazioni 
		{
			...
			for(int i = first_pos;i<end_pos;i++) //divide la matrice in sotto equazioni e risolve ogni sotto equazione per sè
			{
				bool gut = 0;
				bool in_or = 0;
				bool auch = 0;
				...
				for(int j = 0;j<c_size;j++)
				{
					...
					if(last_letter[i]!=-5)
						if(L[i][last_letter[i]] == "o")
						in_or = true;
					
					if(gut)
					break;
					
					if(first_letter[i]!=8000 and !in_or) //se ci sono elementi sulla riga  e se non c'è una sotto-or
						auch = true;
					else
						auch = false;
				...
				cout<<"\n gut : "<<gut<<" auch : "<<auch<<" bien "<<bien<<" riga : "<<i<<" last letter : "<<last_letter[i]<<" string : "<<L[i][last_letter[i]];
				if(!in_or and(gut or auch or bien)) // se gut o auch e se la prossima posizione è la y
		...
		for(int i = 1;i<r_size;i++)
		{
			for(int j = 0;j<c_size;j++)
			{
				if(L[i][j]!="-" and L[i][j]!="o" and L[i][j]!="O" and L[i][j]!="S")
				{
					...
					P[k].r = i;
					P[k].c = 3*j+1;
					cout<<"\nCurrent served contact : "<<L[i][j];
					P[k++].name = L[i][j];
					cout<<"\nCurrent served name : "<<	P[k-1].name;
				}
				if(L[i][j]=="O")
				{
	...
	//	Print_Parts(P,k);
		...
		for(int i = 1;i<r_size;i++)
		{
			for(int j = 0;j<c_size;j++)
			{
				if(L[i][j]!="-" and L[i][j]!="S" and j<col_y)
				{
					//se letterale
					if(L[i][j]!="o" and L[i][j]!="O" )
					{
						cout<<"\nTCurrent served contact : "<<L[i][j];
						P[k].Name_ID = ID++;
						cout<<"\nTCurrent served name : "<<	P[k].name<<" ID : "<<P[k].Name_ID ;
						P[k].Or = false;
						P[k].Contact = true;
						P[k].Coil = false;
						P[k].r = i;
					...
					if(L[i][j]=="o")
					{
						for(int r=i;r>=0;r--)
							if(isNumber(L[r][j]))
								...
								P[k].r = i;
								P[k].c = 3*j+1;
								P[k].Name_ID = P[getPartsIndex(L[r][j],P,r,j*3+1)].Name_ID;
								break;
								}
				...
				cout<<"\nw : "<<L[i][j];
					...
					if(j==col_y and isAlpha(L[i][j]) and L[i][j]!="o" and L[i][j]!="O" )
					{
						P[k].Name_ID = ID++;
						P[k].Or = false;
						P[k].Contact = false;
	...
	/*	for(int k = 0;k<r_size;k++)
		...
		for(int i =0;i<r_w;i++)
			if(isNumber(w[i][power_cont+1]))
				saved_numbers[i][power_cont] = to_string(stoi(w[i][power_cont+1])-1);
		
		cout<<"\nsaved numbers : \n";
		for(int i =0;i<r_w;i++)
			for(int j = 0;j<c_w;j++)
				cout<<saved_numbers[i][j];
	//metti i numeri delle or uguali prima dei contatti interessati
		...
		// se un or
				if(w[i][j]=="o") // se c'è un contatto di "sotto-or" 
				{
					...
					string num = " ";
					bool trovato = false;
					for(int k=j-1;k>=power_cont+1;k--) 
					{
						//  cerca la prima lettera
						if(isAlpha(w[i][k]) and !word)
						{
						r=i;
						c=k-1;
						word=true;
						...
						//  cerca la prima lettera
						if(w[i][k]=="O" or w[i][k]=="o" or k==power_cont and !trovato)
						{	
							num = w[i-1][k+1];  //prendi il numero della riga precedente prima del contatto
						...
						if(trovato)
							{
								cout<<"\ninserisco : "<<num<<" in : "<<w[r][c]<<" posizione : "<<r<<" ; "<<c;
								w[r][c] = num;
							}
			...
			for(int j = power_cont-1;j<c_w;j++)
				{
					...
					{
						if(j!= power_cont or !isNumber(saved_numbers[i][j+2]))
					{
							if(isNumber(w[i][j+1]))
						{
					...
					{
							cout<<"\n Saved num : "<<saved_numbers[i][j+2];
							W[N_Wire++].ID = stoi(saved_numbers[i][j+2])-1; 
							fout<<"\nWire ID : "<<W[N_Wire-1].ID<< " linked to : "<<w[i][j]<<" special";
					}	
					}		
	...
	void Matrix::Associations(){
		...
		for(int i =0;i<r_w;i++)
			for(int j = power_cont;j<c_w;j++)
			{
				...
				{
					index = 0;
					if(j!= power_cont and !isNumber(saved_numbers[i][j+2]))
					{
					if(isNumber(w[i][j+1]))
					...
					{
						index = getWiresIndex(to_string(stoi(saved_numbers[i][j+2])-1),W);
						cout<<"\nIndex got before: "<<index<<" ID : "<<W[index].ID;
					}
					...
					if(col_or[i][j]!=0 and w[i][j]!="o")
						{
							...
							cout<<"\nCardinality at "<<i<<" : "<<j<<" is : "<<max_c[i][j];
							//break;		
						}
				}
...
// Utilities	
	...
	{
		fstream fout;
		fout.open("Parts_new.txt",ios::app);
	...
	void Matrix::Print_Parts_ToFile(Parts P[]){
		...
		fout.open(PARTS_FILE.c_str(),ios::out);
			...
			for(int i=0;i<n_parts;i++)
			{
				if(P[i].Contact and !P[i].Negated)
